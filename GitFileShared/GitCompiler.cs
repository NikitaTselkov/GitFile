...
namespace GitFileShared
{
    public static class GitCompiler
    {
        ...
        public static async Task StartGitFileAsync(string filePath)
        {
            string line;
            ...
            await Task.Run(() =>
            {
                using (StreamReader sr = new StreamReader(filePath, Encoding.Default))
                {
                    ...
                    {
                        _isNeedIgnoreOutput = false;
                        ThreadHelper.JoinableTaskFactory.Run(() => DisplayTextAsync(ex.InnerException?.Message ?? ex.Message));
                    }
                }
            });
        }
                ...
                else if (line.FirstOrDefault() == ':')
                {
                    ...
                    else
                    {
                        ...
                        if (variable != "Empty")
                        {
                            ...
                            ThreadHelper.JoinableTaskFactory.Run(() => DisplayTextAsync($"{variable} = {value} \n"));
                        }
                    }
                }
        ...
        private static string ExecuteCommand(string command)
        {
            ...
            { 
                ...
                {
                    ThreadHelper.JoinableTaskFactory.Run(() => DisplayTextAsync(_outputErrors));
        ...
        private static void ExecuteCommandAndOutputResult(string command)
        {
            ...
            ThreadHelper.JoinableTaskFactory.Run(() => DisplayTextAsync(_output));
        }
        ...
        private static async Task DisplayTextAsync(string text)
        {
